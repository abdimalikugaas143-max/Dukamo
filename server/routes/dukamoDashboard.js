...
router.get('/', async (req, res) => {
  try {
    const [jobs, gigs, workers, employers, applications, bids, cats, recentJobs, recentGigs, countryStats, revenue, messages] = await Promise.all([
      ...
      pool.query("SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE verified=true) as verified FROM worker_profiles"),
      ...
      `),
      pool.query(`
        SELECT jp.id, jp.title, jp.category, jp.job_type, jp.salary_min, jp.salary_max,
               jp.currency, jp.country, jp.is_remote, jp.created_at, ep.company_name
      ...
      `),
      pool.query(`
        SELECT gt.id, gt.title, gt.category, gt.budget, gt.currency, gt.country,
               gt.is_remote, gt.created_at, u.name as poster_name
      ...
      pool.query(`
        SELECT
          COALESCE(SUM(CASE WHEN status='completed' THEN amount ELSE 0 END), 0) AS confirmed_revenue,
    ...
    res.json({
      ...
      verifiedWorkers: Number(workers.rows[0].verified),
      ...
      confirmedRevenue: Number(revenue.rows[0].confirmed_revenue),
      totalGmv: Number(revenue.rows[0].total_gmv),
      totalTransactions: Number(revenue.rows[0].total_transactions),
